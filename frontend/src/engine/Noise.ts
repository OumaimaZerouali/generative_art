 ...
 * Noise Utilities
 *
 * Deterministic noise functions for generating continuous,
 * organic-feeling values across 2D and 3D space.
 */
...
export class SimplexNoise {
   ...
   */
  fbm2D(x: number, y: number, octaves = 4, lacunarity = 2, persistence = 0.5): number {
    let total = 0;
    let frequency = 1;
    let amplitude = 1;
    let maxValue = 0;
   ...
   */
  fbm3D(x: number, y: number, z: number, octaves = 4, lacunarity = 2, persistence = 0.5): number {
    let total = 0;
    let frequency = 1;
    let amplitude = 1;
    let maxValue = 0;
